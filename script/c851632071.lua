...
function s.filter1(c,e)
	return c:IsFaceup() and c:IsSetCard(0xc8) and c:IsMonster() and (c:IsCanBeEffectTarget(e) or c:IsLocation(LOCATION_EXTRA))
end
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return false end
	if chk==0 then return Duel.IsExistingMatchingCard(s.filter1,tp,LOCATION_MZONE|LOCATION_EXTRA,0,1,e:GetHandler(),e)
		and Duel.IsExistingTarget(Card.IsFaceup,tp,0,LOCATION_ONFIELD,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g1=Duel.SelectMatchingCard(tp,s.filter1,tp,LOCATION_MZONE|LOCATION_EXTRA,0,1,1,e:GetHandler(),e)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
