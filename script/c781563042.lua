...
function s.cfilter(c)
	return not (c:IsFaceup() and c:IsCode(CARD_NUMERON_NETWORK))
end
function s.condition(e,tp,eg,ep,ev,re,r,rp)
	return ep==1-tp and #eg==1 and Duel.GetCurrentChain(true)==0 and eg:GetFirst():GetSummonLocation(LOCATION_EXTRA)
...
function s.filter(c,e,tp)
	return c:IsMonster() and c:IsCanBeSpecialSummoned(e,0,tp,true,false,POS_FACEUP,1-tp)
end
