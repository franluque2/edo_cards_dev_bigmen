...
function s.filter(c)
	return c:IsRace(RACE_ZOMBIE) and c:IsAttribute(ATTRIBUTE_WIND) and c:IsFaceup()
end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	local ct=Duel.GetMatchingGroupCount(Card.IsAttribute,e:GetHandler():GetControler(),0,LOCATION_GRAVE,nil,ATTRIBUTE_LIGHT)
