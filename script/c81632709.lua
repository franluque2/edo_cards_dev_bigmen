...
function s.Summonfilter(c)
	return ((((c:IsRace(RACE_FAIRY) and c:IsLevelBelow(4)) and c:IsAttribute(ATTRIBUTE_LIGHT)) and c:IsType(TYPE_NORMAL))) and c:CanBeSpecialSummoned()
end
function s.filter(c)
...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return ft>0 and Duel.IsExistingMatchingCard(s.Summonfilterfilter,tp,LOCATION_HAND+LOCATION_GRAVE,0,1,nil,e,tp) end
