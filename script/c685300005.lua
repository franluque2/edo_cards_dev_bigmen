...
function s.tg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chkc then return chkc:IsLocation(LOCATION_GRAVE|LOCATION_EXTRA) and chkc:IsControler(tp) and s.filter(chkc,e,tp,tid) end
	if chk==0 then return Duel.IsExistingMatchingCard(s.filter,tp,LOCATION_GRAVE|LOCATION_EXTRA,0,1,nil,e,tp,tid) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local max=99
	if Duel.IsPlayerAffectedByEffect(tp,CARD_BLUEEYES_SPIRIT) then max=1 end
	local g=Duel.SelectMatchingCard(tp,s.filter,tp,LOCATION_GRAVE|LOCATION_EXTRA,0,1,max,nil,e,tp,tid)
...
function s.spop1(e,tp,eg,ep,ev,re,r,rp)
    ...
    local sg1=Duel.GetMatchingGroup(s.spfilter,tp,LOCATION_EXTRA,0,nil,e,tp)
    ...
    if #sg1>ft1 then return end
    for tc1 in sg:Iter() do
        Duel.SpecialSummonStep(tc1,0,tp,tp,false,false,POS_FACEUP)
    end
    for tc2 in sg1:Iter() do
...
function s.spop2(e,tp,eg,ep,ev,re,r,rp)
    ...
    local sg1=Duel.GetMatchingGroup(s.spfilter,1-tp,LOCATION_EXTRA,0,nil,e,tp)
	local ft=Duel.GetLocationCount(1-tp,LOCATION_MZONE)
    local ft1=Duel.GetLocationCountFromEx(1-tp)
	if ft>1 and Duel.IsPlayerAffectedByEffect(1-tp,CARD_BLUEEYES_SPIRIT) then ft=1 end
    if ft1>1 and Duel.IsPlayerAffectedByEffect(1-tp,CARD_BLUEEYES_SPIRIT) then ft1=1 end
	if #sg>ft then return end
    if #sg1>ft1 then return end
    for tc1 in sg:Iter() do
