...
function s.initial_effect(c)
	...
	chk1:SetCode(946)
	chk1:SetCondition(s.cusxyzCondition(s.mfilter,4,2,Xyz.InfiniteMats))
	chk1:SetTarget(s.cusxyzTarget(s.mfilter,4,2,Xyz.InfiniteMats))
	chk1:SetOperation(s.cusxyzOperation(s.mfilter,4,2,Xyz.InfiniteMats))
	...
	e1:SetCondition(s.cusxyzCondition(s.mfilter,4,2,Xyz.InfiniteMats))
	e1:SetTarget(s.cusxyzTarget(s.mfilter,4,2,Xyz.InfiniteMats))
	e1:SetOperation(s.cusxyzOperation(s.mfilter,4,2,Xyz.InfiniteMats))
    ...
    e0:SetCode(id)
    e0:SetCondition(function(e,_c) return Duel.IsExistingMatchingCard(Card.IsCode, e:GetHandlerPlayer(), LOCATION_ONFIELD|LOCATION_GRAVE, 0, 1, nil, CARD_ABYSSAL_DREDGE) end)
	...
	e2:SetTarget(function(e,_c) return _c:IsLocation(LOCATION_OVERLAY) and _c:GetReasonCard()==c and not _c:IsReason(REASON_LOST_TARGET) end)
...
function s.mfilter(c,lc,sumtype,tp)
	return c:IsRace(RACE_REPTILE,lc,sumtype,tp) and c:IsAttribute(ATTRIBUTE_DARK,lc,sumtype,tp)
end


function s.negcon(e,tp,eg,ep,ev,re,r,rp)
	return rp==1-tp and Duel.IsChainNegatable(ev) and not e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED)
        and Duel.IsExistingMatchingCard(aux.FaceupFilter(Card.IsCode,CARD_ABYSSAL_DREDGE), tp, LOCATION_ONFIELD, 0, 1, nil)
end
...
function s.negop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.GetMatchingGroup(aux.FaceupFilter(Card.IsCode,CARD_ABYSSAL_DREDGE), tp, LOCATION_ONFIELD, 0, nil)
	if c:IsRelateToEffect(e) and #g>0 then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
		local dg=Duel.SelectMatchingCard(tp,aux.FaceupFilter(Card.IsCode,CARD_ABYSSAL_DREDGE),tp,LOCATION_ONFIELD,0,1,1,nil)
		if #dg>0 then
...
function s.spfilter(c,e,tp)
	return (c:IsCode(CARD_ABYSSAL_DREDGE) or c:ListsCode(CARD_ABYSSAL_DREDGE))
        and c:IsMonster()
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
    ...
    if Duel.IsExistingMatchingCard(s.spfilter, tp, LOCATION_HAND|LOCATION_DECK, 0, 1, nil, e, tp) and Duel.SelectYesNo(tp, aux.Stringid(id,0)) then
        Duel.BreakEffect()
...
function s.cusxyzGetMaterials(tp,xyz)
	return Duel.GetMatchingGroup(function(c)
		if c:IsLocation(LOCATION_GRAVE) and not (c:IsHasEffect(EFFECT_XYZ_MAT_FROM_GRAVE) or c:IsHasEffect(id)) then return false end
