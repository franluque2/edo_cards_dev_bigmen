...
function s.initial_effect(c)
	...
	e4:SetOperation(s.extracon2)
	e4:SetValue(s.extraval2)
...
function s.eftg(e,c)
	return c:IsMonster() and c:IsCanBeLinkMaterial()
end
...
function s.extraval2(chk,summon_type,e,...)
	...
	if chk==0 then
		local tp,sc=...
		if summon_type~=SUMMON_TYPE_LINK or not sc:IsCode(64966519) or Duel.GetFlagEffect(tp,id+1)>0 then
