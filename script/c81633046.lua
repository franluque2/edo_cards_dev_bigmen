--Conscription of the Ancient Bloodline
...
local ARCHETYPE=0x8e

--add the conditions for the archetype swap here
function s.Familiar(c)
  return c:IsCode(34250214)
end

function s.Retainer(c)
  return c:IsCode(70645913)
end

function s.CamulaCards(c)
  return c:IsCode(511000139, 511001903, 100000083, 511000141, 511000143, 511000142)
end
...
function s.flipop(e,tp,eg,ep,ev,re,r,rp)
    ...
    local g=Duel.GetMatchingGroup(s.Familiar, tp, LOCATION_ALL, LOCATION_ALL, nil)
    ...
    g=Duel.GetMatchingGroup(s.Retainer, tp, LOCATION_ALL, LOCATION_ALL, nil)
    ...
    g=Duel.GetMatchingGroup(s.CamulaCards, tp, LOCATION_ALL, LOCATION_ALL, nil)
