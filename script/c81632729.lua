...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	--Destroy 1 spelltrap if Normal Summoned then increase its own DEF if the tribute was a "Doodle Beast" monster
...
function s.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 then
		...
		for gc in aux.Next(g1) do
			atk=atk+gc:GetPreviousAttackOnField()
			def=def+gc:GetPreviousDefenseOnField()
		end

		if Duel.Destroy(tc,REASON_EFFECT)>0 and c:IsRelateToEffect(e) and c:IsSummonType(SUMMON_TYPE_TRIBUTE) and c:IsFaceup() and def>0 then
			Duel.BreakEffect()
			--Increase DEF
			local e1=Effect.CreateEffect(c)
