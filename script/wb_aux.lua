...
function WbAux.CanPlayerSummonDredge(tp)
    ...
    and Duel.IsPlayerCanSpecialSummonMonster(tp,CARD_ABYSSAL_DREDGE,0,TYPE_EFFECT,0,0,4,RACE_REPTILE,ATTRIBUTE_DARK)
end

function WbAux.SpecialSummonDredge(tp, pos)
    if not Duel.IsPlayerCanSpecialSummonMonster(tp,CARD_ABYSSAL_DREDGE,0,TYPE_EFFECT,0,0,4,RACE_REPTILE,ATTRIBUTE_DARK) then return false end
