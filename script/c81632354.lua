--Supreme Machine Magnum Overlord [R] (CT)
local s,id=GetID()
...
function s.filter(c)
	return c:IsFaceup() and c:IsLevelAbove(1) or c:IsRankAbove(1) or c:IsLinkAbove (1)
end
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.DiscardDeck(tp,1,REASON_COST)>0 then
		if c:IsRelateToEffect(e) and c:IsFaceup() then
			...
			e1:SetValue(g:GetFirst():GetLevel()*200 or g:GetFirst():GetRank()*200 or G:GetFirst():GetLink()*200)
