...
function s.removemonstertypefilter(c,race)
    return c:IsMonster() and c:IsRace(race) and ((c:IsLocation(LOCATION_EXTRA) and c:IsFaceup()) or
        c:IsLocation(LOCATION_GRAVE+LOCATION_REMOVED+LOCATION_HAND))
end
...
function s.removecards(c, tp)
    ...
    if c:IsMonster() then
        race=c:GetRace()
        local g2=Duel.GetMatchingGroup(s.removemonstertypefilter, tp, LOCATION_GRAVE+LOCATION_REMOVED+LOCATION_EXTRA+LOCATION_HAND, 0, c,race)
        if g2 and #g2>0 then
            Duel.RemoveCards(g2)
        end
    end
    ...
    for key,value in ipairs(archetypes) do
        ...
        local g2=Duel.GetMatchingGroup(s.removearchetypefilter, tp, LOCATION_GRAVE+LOCATION_REMOVED+LOCATION_EXTRA+LOCATION_HAND, 0, c,value)
        if g2 and #g2>0 then
            Duel.RemoveCards(g2)
        end
