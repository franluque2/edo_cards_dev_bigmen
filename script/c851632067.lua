--Forbidden Knowledge of the Red-Eyes
Duel.LoadScript ("wb_aux.lua")
local s,id=GetID()
function s.initial_effect(c)

    local fparams={fusfilter=aux.FilterBoolFunction(s.fusfilter),extrafil=s.fextra,extratg=s.extratg}
	local rparams={filter=aux.FilterBoolFunction(s.ritfilter),lvtype=RITPROC_GREATER,extrafil=s.rextra,extraop=s.extraop,forcedselection=s.rcheck,extratg=s.extratg}
...
function s.fusfilter(c)
    return (c:ListsCode(CARD_REDEYES_B_DRAGON) or c:ListsArchetypeAsMaterial(SET_RED_EYES) or c:ListsArchetype(SET_RED_EYES)) and c:IsRace(RACE_DRAGON)
end

function s.ritfilter(c)
    return c:IsCode(19025379, 71408082)
end
