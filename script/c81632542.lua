...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if #dg>0 then
		...
		if Duel.Destroy(dg,REASON_EFFECT)>0 and dg:GetFirst():IsMonster() and dg:GetFirst():GetLevel()>0 + dg:GetFirst():GetRank()>0 +dg:GetFirst():GetLink()>0 and Duel.SelectYesNo(tp,aux.Stringid(id,2)) then
			local atk=dg:GetFirst():GetLevel()*200 + dg:GetFirst():GetRank()*200 +dg:GetFirst():GetLink()*200
			if dg:GetFirst():WasMaximumMode() then
				atk=dg:Filter(Card.WasMaximumModeCenter,nil):GetFirst():GetLevel()*200 + dg:GetFirst():GetRank()*200 +dg:GetFirst():GetLink()*200
			end
			local c=e:GetHandler()
