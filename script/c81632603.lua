...
function s.thfilter(c)
	return c:IsCode(511000103) and c:IsAbleToHand()
end
...
function s.thop(e,tp,eg,ep,ev,re,r,rp,chk)
	local tg=Duel.SelectMatchingCard(s.thfilter,tp,LOCATION_DECK,0,nil)
	if tg then
...
function s.Tokenfilter(c,ft)
	return c:IsFaceup() and c:IsType(TYPE_TOKEN) and (ft>0 or c:GetSequence()<5)
end
