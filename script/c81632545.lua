--Marionette Burial (CT)
local s,id=GetID()
function s.initial_effect(c)
	--Special Summon 1 "Doll Chimera" from LOCATIONS
...
function s.cfilter(c,tp)
	return c:IsAbleToGraveAsCost() and c:IsSetCard(0x2517) and Duel.GetMZoneCount(tp,c)>0
end
function s.negfilter(c,tp)
	return c:IsFaceup() and c:IsCode(511001468) and c:IsControler(tp) and c:IsLocation(LOCATION_MZONE)
end
