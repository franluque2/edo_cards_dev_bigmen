...
function s.costfilter(c,code,ft)
	return c:IsFaceup() and c:IsType(TYPE_SPELL+TYPE_CONTINUOUS) and c:IsAbleToGraveAsCost() and (not ft or ft>0 or c:GetSequence()<5) 
end
function s.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return ft>-1 and Duel.IsExistingMatchingCard(s.costfilter,tp,LOCATION_SZONE,0,1,nil,TYPE_SPELL+TYPE_CONTINUOUS)
		...
		and Duel.IsExistingMatchingCard(s.costfilter,tp,LOCATION_MZONE,0,1,nil,TYPE_SPELL+TYPE_CONTINUOUS,ft) end
	...
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g3=Duel.SelectMatchingCard(tp,s.costfilter,tp,LOCATION_MZONE,0,1,1,nil,TYPE_SPELL+TYPE_CONTINUOUS,ft)
	g1:Merge(g2)
	g1:Merge(g3)
