...
function s.shuffledownop(e, tp, eg, ep, ev, re, r, rp)
    ...
    local g2 = Duel.GetMatchingGroup(Card.IsCode, tp, LOCATION_DECK, 0, nil, 170000166)
    if #g2 > 0 then
        Duel.MoveToDeckTop(g2:GetFirst())
    end

    s.bottombigguys(e, tp)
end

local GUYS_TO_BOTTOM = { 82103466, 07634581, 170000169, 170000168, 170000174 }
function s.bottombigguys(e, tp)
    local g1 = Duel.GetMatchingGroup(Card.IsCode, tp, LOCATION_DECK, 0, nil, table.unpack(GUYS_TO_BOTTOM))
    if #g1 > 0 then
...
function s.flipoppassive(e, tp, eg, ep, ev, re, r, rp)
    ...
    e4:SetValue(function(e, te) return te:GetOwnerPlayer() ~= e:GetHandlerPlayer() and
        not (te:IsActiveType(TYPE_MONSTER) and te:GetHandler():IsLevelAbove(8) and te:GetHandler():IsRace(RACE_WARRIOR)) end)
    ...
    e8:SetOperation(s.bottombigguys)
    ...
    e9:SetCondition(s.rewritecallingcon)
    e9:SetOperation(s.rewritecallingop)
...
function s.placecards(e, tp)
    ...
    for i = 1, 2, 1 do
        local gigas = Duel.CreateToken(tp, 681563002)
        Duel.SendtoGrave(gigas, REASON_RULE)
    end
end
...
function s.placecon(e, tp, eg, ep, ev, re, r, rp)
    return tp == rp and
    eg:IsExists(function(c) return c:IsLevelBelow(6) and c:IsControler(tp) and c:GetSummonPlayer() == tp end, 1, nil)
end
...
function s.placeop(e, tp, eg, ep, ev, re, r, rp)
    if Duel.GetLocationCount(tp, LOCATION_SZONE) > 0 and Duel.SelectYesNo(tp, aux.Stringid(id, 1)) then
        ...
        local g = eg:FilterSelect(tp,
            function(c) return c:IsLevelBelow(6) and c:IsControler(tp) and c:GetSummonPlayer() == tp end, 1, 1, nil)
        if #g > 0 then
            ...
            tc:RegisterFlagEffect(id, RESET_EVENT + RESETS_STANDARD, EFFECT_FLAG_CLIENT_HINT, 0, 0,
                aux.Stringid(id, 4))
        end
    end
end
...
function s.nottaggedcallingfilter(c)
    return c:IsCode(170000174) and c:GetFlagEffect(id + 2) == 0
end

function s.rewritecallingcon(e, tp, eg, ep, ev, re, r, rp)
    return Duel.IsExistingMatchingCard(s.nottaggedcallingfilter, tp, LOCATION_ALL, 0, 1, nil)
end

function s.rewritecallingop(e, tp, eg, ep, ev, re, r, rp)
    local g = Duel.GetMatchingGroup(s.nottaggedcallingfilter, tp, LOCATION_ALL, 0, nil)
    for tc in g:Iter() do
        tc:RegisterFlagEffect(id + 2, 0, 0, 0)
